...
const AddNew: React.FC = () => {
    // Get logged-in user's member ID (to be used as introducer)
    const userId = TokenService.getMemberId();

    // Member ID to search for (editable field)
    const [memberId, setMemberId] = useState('');
    const [memberInfo, setMemberInfo] = useState<any>(null);
    const [accountGroupId, setAccountGroupId] = useState<string>('');

    // State for introducer auto-population (when user enters a different ID)
    ...
    const [form, setForm] = useState<any>({
        ...
        maturityDate: '',
        maturityValue: '',
        introducer: userId || '', // Default to logged-in user's ID
        introducerName: '',
        jointMember: '',
    });
    ...
    // Fetch member info when requested (not auto-fetch)
    const { isLoading: loadingMember, refetch: fetchMember } = useGetMemberById(
        memberId,
        false // Don't auto-fetch
    );

    // Fetch logged-in user's info for introducer name
    const { data: loggedInUserData } = useGetMemberById(
        userId || '',
        !!userId
    );
    ...
    // Fetch agent data when introducer code is changed (different from logged-in user) and onBlur triggered
    const { data: agentData, isLoading: isLoadingAgent, isError: isAgentError } = useGetAgentById(
        introducerCode,
        shouldFetchAgent && !!introducerCode && introducerCode.length > 0 && introducerCode !== userId
    );

    // Create account mutation
    const createAccountMutation = useCreateAccountByAgent();

    // Auto-populate introducer name from logged-in user's data on mount
    useEffect(() => {
        if (loggedInUserData?.data && userId) {
            setForm((prev: any) => ({
                ...prev,
                introducer: userId,
                introducerName: loggedInUserData.data.name || '',
            }));
        }
    ...
    // Auto-populate introducer name when agent data is fetched (for different introducer)
    useEffect(() => {
        if (agentData?.data?.name && shouldFetchAgent) {
            setForm((prev: any) => ({
                ...prev,
    ...
    // Handle member info fetch when GET INFO is clicked
    ...
    const handleChange = (field: string, value: any) => {
        ...
        if (field === 'introducer') {
            ...
            // If user clears the introducer field or sets it back to their own ID
            if (!value) {
                setShouldFetchAgent(false);
                setForm((prev: any) => ({
                    ...prev,
                ...
                // Reset to logged-in user's name
                setShouldFetchAgent(false);
                setForm((prev: any) => ({
                    ...prev,
                    introducerName: loggedInUserData?.data?.name || ''
    ...
    // Handle onBlur for introducer field to trigger API call (only if different from logged-in user)
    const handleIntroducerBlur = () => {
        const trimmedIntroducer = form.introducer?.trim();
        if (trimmedIntroducer && trimmedIntroducer.length > 0 && trimmedIntroducer !== userId) {
            setIntroducerCode(trimmedIntroducer);
    ...
    const handleSubmit = async () => {
        ...
        try {
            const accountData = {
                ...
                member_id: memberId, // Use the searched member's ID
                account_type: form.accountType,
                account_operation: form.accountOperation,
                introducer: form.introducer,
                entered_by: userId || undefined, // User as entered_by
                ref_id: form.interestSlab,
                                    ...
                                    <Grid size={{ xs: 12, md: 6 }}>
                                        <FormControl fullWidth size="small" sx={accountInputStyle}>
                                            ...
                                            >
                                                ...
                                                {accountGroups
                                                    .filter((group: any) => ['SB', 'FD', 'PIGMY', 'RD'].includes(group.account_group_name))
                                                    .map((group: any) => (
