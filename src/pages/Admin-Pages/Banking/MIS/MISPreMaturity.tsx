...
const MISPreMaturity: React.FC = () => {
    ...
    const { data: allData } = useGetPreMaturityAccounts(1, 9999, 'AGP006');
    ...
    return (
        <Box sx={{ p: 3, mt: 8 }}>
            <AdminReusableTable
                title="MIS Pre-Maturity Accounts"
                columns={columns}
                data={data?.data || []}
                isLoading={isLoading}
                ...
                emptyMessage="No pre-maturity MIS accounts found"
                actions={
            ...
            <Dialog open={printDialogOpen} onClose={() => setPrintDialogOpen(false)} maxWidth="lg" fullWidth>
                <DialogContent>
                    <Box ref={tablePrintRef}>
                        <TablePDF title="MIS Pre-Maturity Accounts" columns={printColumns} data={printData} />
                    </Box>
                </DialogContent>
                <DialogActions>
